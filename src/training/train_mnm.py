"""
M&M Training Script (Multi-Centre, Multi-Vendor Cardiac MRI)
Based on train_acdc.py with M&M dataset loader
"""
...
from data.mnm_dataset import MnMDataset2D
from losses.sota_loss import CombinedSOTALoss

CLASS_MAP = {0: 'BG', 1: 'LV', 2: 'MYO', 3: 'RV'}
...
def main():
    parser = argparse.ArgumentParser(description="M&M Training")
    parser.add_argument('--data_dir', type=str, default='preprocessed_data/MnM/training')
    ...
    if args.exp_name is None:
        args.exp_name = f"mnm_hrnet_dcn_{datetime.now().strftime('%Y%m%d_%H%M')}"
    
    print(f"\n{'='*60}")
    print("M&M Training - HRNet DCN")
    print(f"{'='*60}")
    ...
    # Data
    dataset = MnMDataset2D(args.data_dir, in_channels=3)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            print(f"E{epoch+1:03d} | Loss: {loss:.4f} | 3D Dice: {metrics['mean_dice']:.4f} | HD95: {metrics['mean_hd95']:.2f}")
