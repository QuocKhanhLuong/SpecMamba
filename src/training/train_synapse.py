"""
Synapse Training Script (Multi-Organ CT)
14 classes: BG, Spleen, Kidneys, Gallbladder, Liver, Stomach, etc.
"""
...
def evaluate_3d(model, dataset, device, num_classes=14):
    """3D Volumetric evaluation for multi-organ."""
    model.eval()
    ...
    for vol_idx in vol_preds.keys():
        ...
        for c in range(1, num_classes):
            ...
            if target_c.sum() > 0:  # Only count organs that exist
                inter = (pred_c & target_c).sum()
                dice = (2 * inter) / (pred_c.sum() + target_c.sum() + 1e-6)
                dice_3d[c].append(dice)
    
    per_class = {c: np.mean(dice_3d[c]) if dice_3d[c] else 0.0 for c in range(1, num_classes)}
    valid_dices = [v for v in per_class.values() if v > 0]
...
def main():
    parser = argparse.ArgumentParser(description="Synapse Training")
    parser.add_argument('--data_dir', type=str, default='preprocessed_data/Synapse/training')
    ...
    # Data
    dataset = SynapseDataset2D(args.data_dir, in_channels=3)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0:
            metrics = evaluate_3d(model, val_ds, device, num_classes=14)
            print(f"E{epoch+1:03d} | Loss: {loss:.4f} | Mean Dice: {metrics['mean_dice']:.4f}")
